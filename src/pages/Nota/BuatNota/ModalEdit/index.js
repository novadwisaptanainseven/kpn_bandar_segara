...
const ModalEdit = ({ isModalOpen, closeModal, setSptTemp, idPelanggan }) => {
  const [value, setValue] = useState("10:00");
  const [marine, setMarine] = useState([]);
  const [driver, setDriver] = useState([]);
  const [tujuan, setTujuan] = useState([]);
  const [loading, setLoading] = useState(false);
  const [dataSptTemp, setDataSptTemp] = useState("");

  useEffect(() => {
    if (isModalOpen.modal) {
      getEditSptTemp(isModalOpen.id, setDataSptTemp);
    }
  }, [isModalOpen]);
  ...
  useEffect(() => {
    if (dataSptTemp) {
      const potonganHarga = hitungTotalHargaSetelahDiskon(
        dataSptTemp.data_spt_temp.diskon,
        dataSptTemp.data_spt_temp.harga_tujuan
      );
    }
  }, [dataSptTemp]);

  const optionsMarine = dataSptTemp
    ? dataSptTemp.data_marine.map((item) => ({
        value: item.id_marine,
        label: `${item.nm_marine}`,
      }))
    : [];

  const optionsTujuan = dataSptTemp
    ? dataSptTemp.data_tujuan.map((item) => ({
        value: item.id_tujuan,
        label: `${item.nm_tujuan}`,
      }))
    : [];

  const optionsDriver = dataSptTemp
    ? dataSptTemp.data_driver.map((item) => ({
  ...
  const handleFormSubmit = (values) => {
    ...
    editSptTemp(
      ...
      dataSptTemp.data_spt_temp.id_pelanggan,
      values,
      setLoading,
      setSptTemp,
      closeModal
      ...
      >
        ...
        <div className=" overflow-scroll" style={{ height: "500px" }}>
          {!dataSptTemp ? (
            ...
            <Formik
              initialValues={initStateEdit(dataSptTemp.data_spt_temp)}
                  ...
                  <ModalBody>
                    <Label className="space-y-1 mt-4">
                      ...
                      <SelectData
                        ...
                        isClearable
                        defaultValue={{
                          label: dataSptTemp.data_spt_temp.nm_marine,
                          value: dataSptTemp.data_spt_temp.id_marine,
                        }}
                      />
                    ...
                    <Label className="space-y-1 mt-4">
                      ...
                      <SelectData
                        ...
                        isClearable
                        defaultValue={{
                          label: dataSptTemp.data_spt_temp.nm_driver,
                          value: dataSptTemp.data_spt_temp.id_driver,
                        }}
                      />
                    ...
                    <Label className="space-y-1 mt-4">
                      ...
                      <SelectData
                        ...
                        isClearable
                        defaultValue={{
                          label: dataSptTemp.data_spt_temp.nm_tujuan,
                          value: dataSptTemp.data_spt_temp.id_tujuan,
                        }}
                      />
